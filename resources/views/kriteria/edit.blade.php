...
<div class="w-full px-6 py-6 mx-auto">
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
        <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6>Form Edit Kriteria</h6>
            <p class="text-sm text-slate-500">Perbarui kriteria untuk perhitungan SAW</p>
        </div>
            ...
            <form action="{{ route('kriteria.update', $kriteria->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="mb-4">
                        ...
                        <input type="text" name="kode_kriteria" required value="{{ old('kode_kriteria', $kriteria->kode_kriteria) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_kriteria" required value="{{ old('nama_kriteria', $kriteria->nama_kriteria) }}"
                            class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-4 py-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                    ...
                    <div class="mb-4">
                        ...
                        <select name="atribut" required
                            ...
                            <option value="benefit" {{ old('atribut', $kriteria->atribut) == 'benefit' ? 'selected' : '' }}>Benefit (Semakin tinggi semakin baik)</option>
                            <option value="cost" {{ old('atribut', $kriteria->atribut) == 'cost' ? 'selected' : '' }}>Cost (Semakin rendah semakin baik)</option>
                        </select>
                        @error('atribut')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="bobot" required min="0" max="1" step="0.01" value="{{ old('bobot', $kriteria->bobot) }}"
                            class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-4 py-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                            placeholder="0.25">
